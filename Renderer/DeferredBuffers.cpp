...
bool DeferredBuffers::Initialize(ID3D11Device* device, int textureWidth, int textureHeight, float screenDepth, float screenNear)
{
	...
	for (i = 0; i<BUFFER_COUNT; i++)
	{
		...
		if (FAILED(result))
		{
			assert(result <= 0 && "Failed to created RenderTarget Texture!");
			return false;
		}
	}
	...
	for (i = 0; i<BUFFER_COUNT; i++)
	{
		...
		if (FAILED(result))
		{
			assert(result <= 0 && "Failed to created RenderTarget View!");
			return false;
		}
	}
	...
	for (i = 0; i<BUFFER_COUNT; i++)
	{
		...
		if (FAILED(result))
		{
			assert(result <= 0 && "Failed to created Shader Resource Views!");
			return false;
		}
	}
...
void DeferredBuffers::SetRenderTargets(ID3D11DeviceContext*deviceContext)
{
	...
	ID3D11ShaderResourceView* pResource[] = {NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL };

	deviceContext->PSSetShaderResources(0, _countof(pResource),pResource);
